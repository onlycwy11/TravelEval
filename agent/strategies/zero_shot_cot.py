...
# 推理指令
在生成最终答案前，请先按以下步骤进行思考，并将思考过程写在最终 JSON 答案之前：

1. 需求分析：解读用户的核心需求、约束条件和偏好。
2. 数据筛选：从提供的数据库中选择合适的景点、酒店和交通方式，并说明理由。
3. 日程编排：构思每日的行程框架，确保时间、路线和活动的合理性。
4. 预算核算：初步估算各项费用，检查是否符合总预算。

完成以上思考后，再严格按照指定格式输出最终规划。
